...
def maxi():
	...
	while 1:
		print("Enter exit for quit this wizard.")
		cmd = str(input(">> "))
		if cmd == "exit":
			break
		elif cmd == "update":
			...
			print("Think after changing settings to restart WTP for these to take effect")
		elif cmd == "dns":
			print("The DNS is a system for replacing the names of files that are very long with domain names that are much shorter and easier to remember.\nThis command is used to access a program that allows you to add, modify and delete DNS entries. You can do this on your peer (127.0.0.1) or on a remote peer. So you can add your own domain name, for free and very easily. Be careful, when you add a new domain name, manually check that it does not already exist!")
		elif cmd == "vpn":
			...
			print("This command is used to send orders by hand to other peers or yours (IP: 127.0.0.1). Most users should not use this command, but it may be useful in some cases.")
			cmd = cmd[7:]
			if cmd == "DemandePresence":
				...
				print("This command allows to receive all the peers known by another peer. It can be very long if the peer knows a lot of peers. If you want to know only a few peers (48 to be precise), use =cmd DemandeNoeud")
			elif cmd == "DemandeListeFichiers":
				print("This command asks a peer for a list of all the files it hosts. The received list is processed and the files are added to the database. Attention, this action could take time if the peer hosts a lot of files.")
			elif cmd == "rechercher":
				...
				print("Unknown command. Enter client then :")
				print("DemandePresence		DemandeNoeud		DemandeFichier")
				print("DemandeListeNoeuds	DemandeListeFichiers	rechercher")
		elif cmd == "majDNS":
			print("This command makes it easy to update your peer's DNS entries with a trusted remote peer (Beware of phishing). The DNS is a system for replacing the names of files that are very long with domain names that are much shorter and easier to remember.")
		elif cmd == "delAll":
			print("This function deletes all settings, all files, the entire WTP database. It's irreversible.")
			...
			print("If you delete the folder or files inside, WTP may not be doing what it is doing or even crashing")
			print("ADDFILES")
			print("This is a folder for you! This is the folder in which you can put all the files you want. After a few minutes, they will be available on the network. If you do not want to wait, you can start the checkFiles command. Once the files on the network, they are deleted from this folder. Remember to save them elsewhere! Attention, so that the files are sent on the network, they must not be in a subfolder of ADDFILES, otherwise they will be removed. Once a file is on the network, no one can delete it. Pay attention !")
			print("HOSTEDFILES")
